...
const exportPDF = () => {
  ...
  // Build HTML document
  let html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>AMC Logs</title><style>'
  html += 'body{font-family:Arial;padding:20px;background:#f5f5f5}'
  html += '.h{text-align:center;margin:0 0 20px;border-bottom:2px solid #4cdbbd;padding:0 0 15px}'
  html += 'h1{color:#0f1419;margin:0;font-size:20px}'
  html += '.info{margin:0 0 20px;background:#fff;padding:15px;border-radius:8px}'
  html += '.row{margin:8px 0}'
  html += 'table{width:100%;border-collapse:collapse;margin:20px 0}'
  html += 'th{background:#0f1419;color:#4cdbbd;padding:12px;border:1px solid #ddd}'
  html += 'td{padding:10px;border:1px solid #ddd}'
  html += '.a{color:#ef4444;font-weight:bold}'
  html += '.n{color:#22c55e;font-weight:bold}'
  html += '.f{margin:30px 0 0;text-align:center;color:#666;font-size:12px;border-top:1px solid #ddd;padding:15px 0 0}'
  html += '</style></head><body>'
  html += '<div class="h"><h1>AMC System Logs Report</h1></div>'
  html += '<div class="info">'
  html += '<div class="row">Date: ' + exportDate + '</div>'
  html += '<div class="row">Entries: ' + logs.length + '</div>'
  html += '<div class="row">Panels: ' + selectedPanelsText + '</div>'
  html += '<div class="row">Filter: ' + statusFilterText + '</div>'
  html += '</div>'
  html += '<table><tr><th>Time</th><th>Panel</th><th>Status</th><th>Details</th></tr>'

  logs.forEach(log => {
    const cls = log.status === 'ALARM' ? 'a' : 'n'
    html += '<tr><td>' + log.time + '</td><td>' + log.room + '</td><td><span class="' + cls + '">' + log.status + '</span></td><td>' + log.detail + '</td></tr>'
  })

  html += '</table><div class="f">Generated by AMC (AirNav Monitoring & Control)</div></body></html>'
