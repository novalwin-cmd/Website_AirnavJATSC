...
const exportPDF = () => {
  ...
  // Generate table rows - escape special characters
  const tableRows = logs.map(log => {
    const statusClass = log.status === 'ALARM' ? 'alarm' : 'normal'
    const escapeHtml = (text) => text.replace(/[&<>"']/g, char => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[char]))
    return '<tr><td>' + escapeHtml(log.time) + '</td><td>' + escapeHtml(log.room) + '</td><td><span class="' + statusClass + '">' + escapeHtml(log.status) + '</span></td><td>' + escapeHtml(log.detail) + '</td></tr>'
  }).join('')

  const exportDate = new Date().toLocaleString()
  const statusFilterText = logFilter.value === 'all' ? 'All Events' : logFilter.value === 'alarms' ? 'Alarms Only' : 'Normal Events'
  const htmlContent = '<!DOCTYPE html><html><head><title>AMC System Logs Report</title><style>body{font-family:Arial,sans-serif;padding:20px;background:#f5f5f5}.header{text-align:center;margin-bottom:30px;border-bottom:2px solid #4cdbbd;padding-bottom:15px}.header h1{color:#0f1419;margin:0;font-size:24px}.header p{color:#666;margin:5px 0}.info{margin-bottom:20px;background:white;padding:15px;border-radius:8px}.info-row{margin:8px 0}.info-label{font-weight:bold;color:#333}table{width:100%;border-collapse:collapse;background:white;margin-top:20px}th{background:#0f1419;color:#4cdbbd;padding:12px;text-align:left;font-weight:bold;border:1px solid #ddd}td{padding:10px 12px;border:1px solid #ddd}tr:nth-child(even){background:#f9f9f9}tr:hover{background:#f0f0f0}.alarm{color:#ef4444;font-weight:bold}.normal{color:#22c55e;font-weight:bold}.footer{margin-top:30px;text-align:center;color:#666;font-size:12px;border-top:1px solid #ddd;padding-top:15px}</style></head><body><div class="header"><h1>⚡ AMC System Logs Report</h1><p>AirNav Monitoring & Control - System Log Export</p></div><div class="info"><div class="info-row"><span class="info-label">Exported Date:</span> ' + exportDate + '</div><div class="info-row"><span class="info-label">Total Entries:</span> ' + logs.length + '</div><div class="info-row"><span class="info-label">Panels Included:</span> ' + selectedPanelsText + '</div><div class="info-row"><span class="info-label">Status Filter:</span> ' + statusFilterText + '</div></div><table><thead><tr><th>Time</th><th>Panel</th><th>Status</th><th>Details</th></tr></thead><tbody>' + tableRows + '</tbody></table><div class="footer"><p>This report was generated by AMC (AirNav Monitoring & Control) System</p><p>© 2026 AirNav. All rights reserved.</p></div><script>window.print();</script></body></html>'
